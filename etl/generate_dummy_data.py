...
TAHUN_AKADEMIK = [
    (1,'2020/2021-1',2020,1), (2,'2020/2021-2',2020,2),
    (3,'2021/2022-1',2021,1), (4,'2021/2022-2',2021,2),
    (5,'2022/2023-1',2022,1), (6,'2022/2023-2',2022,2),
    (7,'2023/2024-1',2023,1), (8,'2023/2024-2',2023,2),
    (9,'2024/2025-1',2024,1), (10,'2024/2025-2',2024,2),
]
...
for i in range(1, 101):
    ...
    dosen_rows.append((
        i, f"{1000000000 + i:010d}", fake.name(), prodi_rand[0],
        random.choice(JABATAN), random.choice(PENDIDIKAN),
        fake.date_between(start_date=date(2000,1,1), end_date=date(2020,1,1)), True
    ))
...
for i in range(1, 1501):
    ...
    status = 'Aktif' if tahun_masuk >= 2022 else random.choices(
        ['Aktif','Lulus','DO','Cuti'], weights=[0.5,0.4,0.05,0.05])[0]
    
    mahasiswa_rows.append((
        i, nim, fake.name(), jk,
        fake.date_of_birth(minimum_age=18, maximum_age=28),
        prov[0], f"SMA Negeri {random.randint(1,50)} {prov[1]}",
        akr_sma, # Disimpan di memori index ke-7 untuk kalkulasi nilai, tidak di-insert ke DB
        prodi[0], tahun_masuk, status
    ))
...
for mhs in mahasiswa_rows:
    ...
    ta_match = next((ta for ta in ta_rows if ta[2] == tahun_masuk and ta[3] == 1), random.choice(ta_rows))
...
for mhs in mahasiswa_rows:
    ...
    if status == 'Aktif' and tahun_masuk >= 2022: n_smt = random.randint(2, 4)
    elif status == 'Lulus': n_smt = random.randint(7, 9)
    elif status == 'DO': n_smt = random.randint(1, 4)
    else: n_smt = random.randint(4, 6)
    
    available_ta = [ta for ta in ta_rows if ta[2] >= tahun_masuk][:n_smt]
    mks_prodi = mk_by_prodi.get(id_prodi, mk_rows[:15])
    dosens_prodi = dosen_by_prodi.get(id_prodi, dosen_rows[:5]) or dosen_rows[:5]
    
    ipk_running = 0.0
    sks_total = 0
    bobot_total = 0.0
    
    for ta in available_ta:
        ...
        for mk in smt_mks:
            ...
            krs_rows.append((krs_id, id_mhs, id_mk, ta[0], dosen[0], tgl_krs, 'Disetujui'))
            
            # Pengaruh kualitas sekolah thd probabilitas nilai
            if akr_sma == 'A': nilai_probs_adj = [0.20,0.18,0.18,0.16,0.10,0.07,0.05,0.04,0.02]
            elif akr_sma == 'B': nilai_probs_adj = NILAI_PROBS
            else: nilai_probs_adj = [0.08,0.10,0.14,0.18,0.16,0.12,0.10,0.08,0.04]
...
# Save to PostgreSQL
# ─────────────────────────────────────────────
print("\nMenyimpan data ke PostgreSQL (siakad_oltp)...")
...
def insert_to_pg(table, rows):
    if not rows: return
    placeholders = ','.join(['%s'] * len(rows[0]))
    query = f"INSERT INTO {table} VALUES ({placeholders})"
    cur.executemany(query, rows)
    print(f"  Tersimpan di tabel {table}: {len(rows)} baris")

insert_to_pg('provinsi', provinsi_rows)
insert_to_pg('fakultas', fakultas_rows)
insert_to_pg('program_studi', prodi_rows)
insert_to_pg('tahun_akademik', ta_rows)
insert_to_pg('dosen', dosen_rows)
insert_to_pg('mata_kuliah', mk_rows)

# Hapus index 7 (akreditasi_sma_asal) dari tupple mahasiswa khusus untuk insert ke database
mahasiswa_pg_rows = [m[:7] + m[8:] for m in mahasiswa_rows]
insert_to_pg('mahasiswa', mahasiswa_pg_rows)

insert_to_pg('pendaftaran', pendaftaran_rows)
insert_to_pg('krs', krs_rows)
insert_to_pg('nilai_mahasiswa', nilai_rows)

conn.commit()
cur.close()
conn.close()

print("\nSelesai! Dummy data OLTP sukses terisi penuh di PostgreSQL.")